...
passport.use(new TwitterStrategy({
  ...
  function(token: string, tokenSecret: string, profile: string, cb: any) {
    return cb(null, {
      profile: profile,
      access_token: token,
 ...
 */
auth.get_twitter_auth_url = (host: string, callback_action: any) => {
  ...
  return new Promise (function (resolve, reject) {
    request(request_options, (error: any, response: any) => {
      if (error) {
        reject(error)
      }
      else {
